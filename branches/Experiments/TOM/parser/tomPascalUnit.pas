...
interface
uses
  Sysutils, Classes, Contnrs, CastaliaPasLex,CastaliaPasLexTypes;
...
TTokenItem = class(TObject)
  ...
  public
    constructor Create(aPasLex:TmwPasLex); overload;
...
constructor TTokenItem.Create(aPasLex:TmwPasLex);
begin
  FTokenKind := aPasLex.TokenID;
  FTokenValue := aPasLex.Token;
...
begin
  ...
  try
    ...
    begin
        ...
        if (not FParsePastEndPeriod) and (Lex.TokenID = ptEnd) and (Lex.AheadTokenID = ptPoint) then
        begin
          Lex.Next; // Goto Period
          Add(TTokenItem.Create(Lex));
          Break;
        end;
        Lex.Next;
